...
def main():
    ...
    print("Optimizing dimensionality...")
    pca = PCA(n_components=min(128, len(embeddings[0])))
    ...
    cosine_dist = np.asarray(cosine_dist, dtype=np.float64)
    
    print("Clustering with optimized parameters...")
    ...
    for label in unique[counts < 5]:  # Merge very small clusters
        labels[labels == label] = -1
    
    # Relabel to consecutive numbers excluding noise (-1)
    ...
    if len(set(labels)) > 1:
        ...
        if sil_score < 0.15:
            print("Warning: Low silhouette score - trying KMeans fallback")
            k = min(10, len(embeddings)//10)
