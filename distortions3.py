...
def main():
    ...
    # 1. Apply PCA with optimal components
    optimal_components = min(75, len(embeddings[0]))
    ...
    # 3. Enhanced clustering with hybrid approach
    print("Running optimized clustering...")
    
    # First try HDBSCAN with cosine similarity
    
    cosine_dist = cosine_distances(embeddings).astype(np.float64)
    
    hdb = hdbscan.HDBSCAN(
        min_cluster_size=max(3, int(len(df)*0.005)),  # ~0.5% of data
        min_samples=2,  # Reduced from 3
        metric='precomputed',
        cluster_selection_method='eom',
        cluster_selection_epsilon=0.5,  # Balanced value
        alpha=0.7  # Slightly less aggressive than 1.0
    )
    hdb_labels = hdb.fit_predict(cosine_dist)
    
    # Evaluate HDBSCAN results
    if len(set(hdb_labels)) > 1:
        hdb_score = silhouette_score(embeddings, hdb_labels, metric='cosine')
        print(f"HDBSCAN Silhouette: {hdb_score:.4f}")
        
        if hdb_score >= 0.15:
            labels = hdb_labels
        else:
            print("HDBSCAN score low - trying optimized KMeans")
            # Determine optimal clusters
            
            possible_k = range(4, min(15, len(embeddings)//10))
            best_k = 4
            best_score = -1
            ...
            for k in possible_k:
                ...
                score = silhouette_score(embeddings, km_labels, metric='cosine')
        ...
        print("HDBSCAN failed - using optimized KMeans")
        labels = KMeans(n_clusters=8, random_state=42).fit_predict(embeddings)
