...
def extract_cluster_insights(df, model, embeddings, cluster_col="cluster", text_col="cleaned_text", top_n_keywords=10, output_folder="Cluster_Outputs"):
    ...
    for cluster_id in tqdm(sorted(df[cluster_col].unique())):
        ...
        cluster_summary.append({
            ...
            "keywords": keywords,
            "representative_text": representative_comment
        })

        # Create DataFrame and handle empty case
    if not cluster_summary:
        return pd.DataFrame(columns=["cluster_id", "cluster_size", "keywords", "representative_text"])
    
    result_df = pd.DataFrame(cluster_summary)

    return pd.DataFrame(cluster_summary).sort_values("cluster_size", ascending=False).reset_index(drop=True)
...
def main():
    ...
    n_components = min(100, len(df)-1)
    pca_model = PCA(n_components=n_components)
    pca_100 = pca_model.fit_transform(embeddings)
    ...
    labels = clusterer.fit_predict(pca_100)
    ...
    if len(unique_labels) < 2:
        ...
        if sum(valid_idx) > 1:  # Need at least 2 samples
            sil_score = silhouette_score(pca_100[valid_idx], labels[valid_idx])
